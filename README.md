...
The orchestrator script supports multiple distinct output formatting modes tailored for terminal reading or downstream tool ingestion (e.g., **OpenClaw**):
...
```bash
./venv/bin/python3 main.py --pretty
```
...
```bash
./venv/bin/python3 main.py --human
```
...
```bash
./venv/bin/python3 main.py --auth --human
```
...
```bash
./venv/bin/python3 main.py --auth --headless
```
